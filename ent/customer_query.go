...
func (cq *CustomerQuery) loadAccounts(ctx context.Context, query *AccountQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Account)) error {
	...
	query.Where(predicate.Account(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.AccountsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_accounts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
